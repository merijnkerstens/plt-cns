...
def get_promoter_bed_per_gene(cds_bed):
    ...
    cmd = f"bedtools flank -i {cds_bed} -g {base}PLAZA5.0-files-monocots/genomes/{species}.bed -l 20000 -r 0 -s > {promoter_bed}"
    result = subprocess.run(cmd, shell=True)
    return promoter_bed

def update_promoter_bed_upstream_gene(cds_promoter):
    species = cds_promoter.split('.')[0]
    cmd = f"bedtools intersect -a {base}PLAZA5.0-files-monocots/gffs/{species}/annotation.selected_transcript.all_features.{species}.gff3.tmp -b {cds_promoter}"
...
def get_all_plts():
    list_files = [f for f in os.listdir("/lustre/BIF/nobackup/boele028/SA_2023/promoters_PLT/111124_20kb-cds_plt-chopped/prepping_promoters_monocots/") if f.endswith('.list')]
    all_plts = []
    for file in list_files:
        with open(f"{base}prepping_promoters_monocots/{file}", 'r') as f:
            line = f.readlines()
            for elem in line:
...
if __name__ == "__main__":
    ...
    gffs = '/lustre/BIF/nobackup/boele028/SA_2023/promoters_PLT/111124_20kb-cds_plt-chopped/PLAZA5.0-files-monocots/gffs/*/annotation.selected_transcript.all_features.*.gff3'
